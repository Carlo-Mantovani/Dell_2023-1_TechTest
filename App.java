...
public class App {
    ...
    private static List<String> listCities(Map<String, Map<String, Integer>> routes) {
        ...
        String option = "";
        while (!option.equals("N")) {//enquanto o usuario nao digitar N, o programa continua a pedir para o usuario digitar as cidades do trajeto
            System.out.print("Digite o nome da cidade a ser cadastrada no trajeto: ");//pede para o usuario digitar o nome da cidade
            ...
            if (cities.size() <= 1) {//verifica se a cidade foi a primeira a ser cadastrada, para garantir que pelo menos duas cidades foram cadastradas
            ...
            System.out.print("Desejas adicionar outra cidade? (S/N): ");//pergunta se o usuario deseja adicionar mais uma cidade
            option = kb.nextLine();
            option = option.toUpperCase();
            if (!option.equals("N") && !option.equals("S")) {//verifica se a opcao digitada eh valida
                while (!option.equals("N") && !option.equals("S")) {//enquanto a opcao digitada nao for valida, o programa pede para o usuario digitar novamente
    ...
    private static List<Product> listProducts() {
        ...
        String option = "";
        while (!option.equals("N")) {//enquanto o usuario nao digitar N, o programa continua a pedir para o usuario digitar os produtos a serem transportados
            System.out.print("\nDigite o nome do produto a ser adicionado no transporte: ");//pede para o usuario digitar o nome do produto
            ...
            System.out.println("Digite o quantidade do produto: ");//pede para o usuario digitar a quantidade do produto
            ...
            Product product = new Product(prodName, prodWeight, prodQuantity);//cria um novo produto com os dados digitados pelo usuario
    ...
    //metodo responsavel por verificar se serao feitos depositos em alguma cidade intermediaria
    private static Map<String, Map<String, Integer>> deposit(List<String> cities, List<Product> products) {

        //mapa que armazena os depositos a serem feitos em cada cidade, sendo que a chave eh o nome da cidade e o valor eh um mapa que armazena o nome do produto e a quantidade a ser depositada
        Map<String, Map<String, Integer>> deposit = new HashMap<String, Map<String, Integer>>();

        List<Product> auxProducts = new LinkedList<>();//lista auxiliar que armazena os produtos a serem transportados para que nao seja alterada a lista original
        for (Product p : products) {

            auxProducts.add(new Product(p));
        }

        System.out.println("Sera feito algum deposito em alguma cidade? (S/N): ");//pergunta ao usuario se serao feitos depositos em alguma cidade
        String option = kb.nextLine();
        option = option.toUpperCase();

        while (option.equals("S")) {//enquanto o usuario digitar S, o programa continua a pedir para o usuario digitar as cidades que receberao depositos

            System.out.println("Digite o nome da cidade intermediária a se fazer depósito: ");//pede para o usuario digitar o nome da cidade que recebera o deposito
            ...
            System.out.println("Digite o nome do produto: ");//pede para o usuario digitar o nome do produto que sera depositado
            ...
            if (!prodFound) {//se o produto nao for encontrado, o programa pede para o usuario digitar novamente
            ...
            System.out.println("Digite a quantidade de deposito (Quantidade Atual: "
                    + currentProductQuantity(prodName, auxProducts) + "): ");//pede para o usuario digitar a quantidade de deposito, demonstrando a quantia atual do produto
            ...
            if (depositQuantity <= 0 || !checkDepositLimit(prodName, depositQuantity, products)) {//verifica se a quantidade digitada eh valida
                System.out.print("Quantidade invalida, tente novamente.\n");
                continue;
            }
            ...
            if (deposit.containsKey(cityName)) {//verifica se a cidade ja recebeu algum deposito
                ...
                if (prodDeposit.containsKey(prodName)) {//verifica se o produto ja foi depositado na cidade
                    ...
                    int currentQuantity = prodDeposit.get(prodName);//pega a quantidade atual de deposito do produto na cidade
                    prodDeposit.put(prodName, currentQuantity + depositQuantity);//atualiza a quantidade de deposito do produto na cidade

                } else {//se o produto nao foi depositado na cidade, o programa adiciona o produto na lista de depositos da cidade
                    prodDeposit.put(prodName, depositQuantity);
                }
            } else {//se a cidade nao recebeu nenhum deposito, o programa adiciona a cidade na lista de depositos
            ...
            System.out.println("Desejas fazer mais algum deposito? (S/N): ");//pergunta ao usuario se ele deseja fazer mais algum deposito
            option = kb.nextLine();
            option = option.toUpperCase();
            if (!option.equals("N") && !option.equals("S")) {//verifica se a opcao digitada eh valida
                while (!option.equals("N") && !option.equals("S")) {//enquanto a opcao digitada nao for valida, o programa pede para o usuario digitar novamente
    ...
    //metodo que verifica se a quantidade digitada excede a quantidade de produtos disponiveis
    ...
    public static void main(String[] args) { //main
        ...
        //leitura do arquivo de rotas e armazenamento em um mapa. Segue o formato: <cidade de origem, <cidade de destino, distancia>>
        Map<String, Map<String, Integer>> routes = new HashMap<String, Map<String, Integer>>(); 
        fr.readFile(data, routes);

        //leitura do arquivo de custos e armazenamento em um mapa. Segue o formato: <modalidade de transporte, custo>
        ...
        while (true) {
            ...
            switch (opt) {
                ...
                case "1":
                    ...
                    origin = origin.toUpperCase();//converte a cidade de origem para maiusculo para ser comparada com as cidades do mapa de rotas
                    ...
                    if (!routes.get(origin).containsKey(destination)) { //verifica se a cidade de destino existe no mapa de rotas
                    ...
                    switch (vehicle_type) {//imprime o custo do trecho de acordo com o tipo de veiculo, realizando o calculo de custo x distancia
                        case "1":
                            cost = String.format("%.2f", (costs.get(1) * routes.get(origin).get(destination)));//formata o custo para duas casas decimais
                            ...
                            break;
                        case "2":
                            cost = String.format("%.2f", (costs.get(2) * routes.get(origin).get(destination)));//formata o custo para duas casas decimais
                            System.out.print("De " + origin + " para " + destination
                            ...
                            break;
                        case "3":
                            cost = String.format("%.2f", (costs.get(3) * routes.get(origin).get(destination)));//formata o custo para duas casas decimais
                            System.out.print("De " + origin + " para " + destination
                ...
                case "2":
                    ...
                    if (cities.size() > 2) {//se a quantidade de cidades for maior que 2, o usuario pode cadastrar depositos em cidades intermediarias
                    ...
                    Transport newTransport = new Transport(cities, products, deposit, company);//cria um novo transporte
                    transports.add(newTransport);//adiciona o transporte a lista de transportes
     
                    System.out.println(newTransport.toString(routes, costs));//imprime os dados do transporte cadastrado
